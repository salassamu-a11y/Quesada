...
| GET    | /taller?k=TALLER_TOKEN            | **PÚBLICA, fuera del bloque `/admin`**: pantalla del taller (citas `confirmada`/`atendida` de hoy en dos bloques, con KM —o PRECIO en los pinchazos— + ACABADO y NO SE HACE en las `atendida`). Token incorrecto/ausente/no configurado → 404 genérico (ver "Pantalla del taller") |
...
| POST   | /taller/acabar                    | **PÚBLICA, fuera del bloque `/admin`**: escritura desde la pantalla del taller. Body form-urlencoded `k` (mismo `TALLER_TOKEN`) + `id` + `kilometros` y/o `precio` (ambos opcionales para el servidor; el precio lo manda la pantalla en los pinchazos). Solo pasa a `acabada`, solo desde `confirmada`/`atendida` y **solo si la cita es de HOY**; guarda km y precio si vienen (inválidos → 409). Token malo → 404 genérico en texto plano; cita inexistente → 404 HTML; cualquier otro caso → 409 HTML; éxito → 302 a `/taller?k=` (ver "Pantalla del taller") |
| POST   | /taller/incidencia                | **PÚBLICA, fuera del bloque `/admin`**: clon de `/taller/acabar` con las mismas reglas (token, 404/409, solo desde `confirmada`/`atendida`, solo HOY) pero pasa a `incidencia` y guarda `motivo` si viene (≤100 car.). No pide km. Preparación común en `prepararEscrituraTaller()` (ver "Pantalla del taller") |
...
| PUT    | /admin/cita/:id                   | Edita los datos de una cita existente. Misma `validarCita` que el alta pero con `permitirPasado=true`. **Conserva `id`, `creadaEn`, `estado` y `recordatorioEnviado`.** Hace `delete` explícito de los campos retirados `tipoVehiculo` y `factura` (ver "Modelo de cita"). 404 si no existe; 400 si falla validación |
| POST   | /admin/cita/:id/estado            | Cambia estado (pendiente/confirmada/atendida/acabada/llamado/incidencia/pagada/cancelada). Lo usan tanto el desplegable como el botón ✓ del listado. Acepta además `vista` y `semana` (hidden del formulario, pasan por `resolverVista` y `lunesDesdeParam`) y responde **302 a la misma vista/semana con ancla `#cita-<id>`** para no perder la posición del scroll (ver "Botón ✓"). 400 si body inválido o estado no válido; 404 si la cita no existe |
...
- **Desplegable de estado**: ofrece `confirmada`, `atendida`, `acabada`, `llamado`, `incidencia`, `pagada` y `cancelada`; `pendiente` aparece únicamente si la cita ya está en ese estado. La validación del servidor (`POST /admin/cita/:id/estado`) acepta los ocho. Es la vía para **corregir y retroceder**; el avance normal se hace con el botón ✓.
...
- **Copiar al Excel** (commits `c179f86` + `17c9da2`): sustituye al plan de exportar CSV (ver "Pendiente"). Cada `<tr>` del listado lleva **`data-excel`** con las **12 columnas del `Diario.ods`** del taller, **separadas por TABULACIÓN** (ni comas ni punto y coma: la tabulación es lo que Excel/LibreOffice interpretan como salto de columna al pegar), en su orden: FECHA (`dd/mm/aa`), MARCA (`vehiculo`), MATRICULAS, DESCRIPCIÓN (servicio + " — " + detalle), NOMBRE CLIENTE, NUMERO (teléfono), PAGO, M VEHICULO (km), PO SI, FACTURA, IMPORTE (precio), FECHA COBRO. Sin cabeceras. Tabs y saltos de línea dentro de un campo pasan a espacio (`celdaExcel`); en el atributo el tab viaja como `&#9;`.
  ...
  - **Tres columnas van VACÍAS**: PO SI, FACTURA y FECHA COBRO, que el panel no guarda. Se dejan en su sitio para que al pegar cuadren las columnas; son las que Vicky rellena a mano.
  ...
  - **IMPORTANTE para el usuario: hay que pegar situándose en la COLUMNA A** de la hoja. Si se pega en otra columna, las 12 se desplazan y dejan de cuadrar con las cabeceras del `Diario.ods`.
...
- **Celdas y bloques pulsables** (04/09/2026):
  ...
  - **Bloque de cita → edición**: `onclick="editarCita(this)"` con los **mismos 12 `data-*`** que el botón Editar del listado (`id`, `nombre`, `telefono`, `fecha`, `hora`, `servicio`, `detalle`, `matricula`, `vehiculo`, `kilometros`, `precio`, `pago`; eran 14 antes de retirar `tipo-vehiculo` y `factura`), misma función, mismo formulario y mismo `PUT`. Realce con anillo (`hover:ring-1`) para no alterar el color de estado del fondo.
...
- **Datos que muestra**: hora, **NOMBRE DE PILA** (lo anterior al primer espacio del nombre completo), servicio, detalle, matrícula (pastilla con borde), vehículo y, en las `atendida`, los **kilómetros** (campo KM prerrellenado si Vicky ya los puso). El tipo de vehículo **ya no se muestra**: el campo `tipoVehiculo` se retiró (ver "Modelo de cita"). **PROHIBIDO**: precio (**salvo el campo "€" VACÍO a rellenar en los pinchazos**, ver "Fila 1"; nunca un importe guardado), **forma de pago (`pago`)** — dato económico —, apellidos, teléfono e id. Los ve el cliente que espera y cualquiera que pase. **Los km SÍ se ven desde que existe el campo KM** (antes estaban en la lista de prohibidos): **asumido**, el kilometraje no es dato sensible como el precio. Sin enlaces a `/admin` ni a ninguna otra vista: **callejón sin salida a propósito**.
...
- **`POST /taller/acabar` y `POST /taller/incidencia`** — las **DOS ÚNICAS escrituras** desde la pantalla. Rutas públicas, fuera del bloque `/admin`, mismo `TALLER_TOKEN` (en el body) y mismo `safeEqual` que el GET; token malo → **mismo 404 genérico en texto plano**. Sin `isSameOrigin` (el secreto es el token). Reglas y razón del diseño en "Seguridad — panel /admin". La parte común (token + `parseBody` + `volver` + `errorHtml`) está extraída en **`prepararEscrituraTaller()`**, que devuelve `null` si ya respondió el 404 o `{ body, volver, errorHtml }`. Flujo de cada uno:
  - Lectura fresca con `readCitas()` y **parcheo de UN SOLO registro** (regla del proyecto): `estado` y, si vienen, `kilometros` (acabar) o `motivo` (incidencia); ni `id`, ni `creadaEn`, ni `recordatorioEnviado` ni nada más.
  - Cita inexistente → **404**; estado distinto de `confirmada`/`atendida` **o fecha distinta de HOY** → **409** sin tocar nada (cubre el doble clic: la segunda pulsación ya está en `acabada`/`incidencia`). En `acabar`, km con contenido inválido → **409** sin tocar nada; km vacíos → se marca `acabada` igualmente y `cita.kilometros` no se toca. Misma regla para el motivo en `incidencia`.
...
- **EXPORTACIÓN AL EXCEL del taller — HECHA como "Copiar al Excel"** (commits `c179f86` + `17c9da2`, ver "Panel /admin — vista de citas"). El taller lleva un **`Diario.ods`** (LibreOffice) con estas columnas: **FECHA, MARCA, MATRICULAS, DESCRIPCIÓN, NOMBRE CLIENTE, NUMERO** (teléfono), **PAGO, M VEHICULO** (km), **PO SI, FACTURA, IMPORTE, FECHA COBRO**. Antes Vicky lo copiaba a mano desde el panel; ahora cada fila lleva un botón que copia la cita al portapapeles con esas 12 columnas por tabulación (y "Copiar todas" para las filas visibles), y ella la pega en la columna A de la hoja.
  ...
  - **Sigue SIN RESPUESTA de Carles**: qué significan las columnas **"PO SI"** y **"FECHA COBRO"**. Son las **únicas dos que Vicky rellenará a mano** (la copia las deja vacías junto con FACTURA, cuyo campo se retiró del panel). Ya no bloquea nada: si algún día lo aclaran y el panel puede guardarlas, se rellenan en `lineaExcel`.
- **BORRADO AUTOMÁTICO de citas pasado un plazo** (minimización de datos, RGPD): el histórico real lo llevan en su Excel, el panel no necesita conservar citas viejas. **DECIDIDO: 30 días** (Vicky había propuesto una semana; Samuel un mes por si algún día no da tiempo a pasarlo al Excel). **FALTA IMPLEMENTARLO.**
...
- La pantalla `/taller` es **pública por diseño**: cualquier dato nuevo que se añada ahí debe pasar el filtro "¿puede verlo un cliente que espera?". Precio, **forma de pago**, apellidos y teléfono, **nunca**. **Matiz del precio**: en los pinchazos aparece un campo "€" para que el mecánico lo escriba, pero **siempre VACÍO**, jamás prerrellenado con el importe guardado; mostrar un precio existente sigue prohibido. Los km sí se muestran (campo KM de las `atendida`, prerrellenado): asumido como dato no sensible.
