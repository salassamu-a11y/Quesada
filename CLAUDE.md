...
| GET    | /taller?k=TALLER_TOKEN            | **PÚBLICA, fuera del bloque `/admin`**: pantalla del taller (citas `confirmada`/`atendida` de hoy en dos bloques, con KM + ACABADO y NO SE HACE en las `atendida`). Token incorrecto/ausente/no configurado → 404 genérico (ver "Pantalla del taller") |
| POST   | /taller/acabar                    | **PÚBLICA, fuera del bloque `/admin`**: escritura desde la pantalla del taller. Body form-urlencoded `k` (mismo `TALLER_TOKEN`) + `id` + `kilometros` (opcional para el servidor). Solo pasa a `acabada`, solo desde `confirmada`/`atendida` y **solo si la cita es de HOY**; guarda los km si vienen (inválidos → 409). Token malo → 404 genérico en texto plano; cita inexistente → 404 HTML; cualquier otro caso → 409 HTML; éxito → 302 a `/taller?k=` (ver "Pantalla del taller") |
...
| POST   | /admin/cita/:id/estado            | Cambia estado (pendiente/confirmada/atendida/acabada/llamado/incidencia/pagada/cancelada). Lo usan tanto el desplegable como el botón ✓ del listado. 400 si body inválido o estado no válido; 404 si la cita no existe |
...
- **Color de fila y badges** — **solo destacan los estados que reclaman acción de Vicky** (llamar al cliente), si no el color pierde sentido:
  ...
  - Badges de la columna Estado (`estadoBadge`): confirmada verde, **atendida azul** (`blue-900/50` / `blue-300`), **acabada amarillo sólido** (`bg-[#FFD700]` con texto navy), **incidencia rojo sólido** (`bg-red-600` texto blanco), **llamado azul apagado** (`slate-800/50` / `slate-400`, distinto del gris de pagada), **pagada gris** apagado, cancelada rojo apagado (distinto del sólido de incidencia), pendiente amarillo oscuro (distinto del sólido de acabada). Las mismas clases pintan el fondo de cada cita en el calendario.
...
Rejilla semanal propia (`calendarioHTML(citas, lunes, hoy)`), **NO la tabla del listado**. Vista completa: cabecera, banda de acabados y "+ Nueva cita" siguen; la tabla no aparece debajo. **Solo muestra**: no se crea cita pinchando en un hueco.
...
| Pantalla taller | ✅   | `GET /taller?k=TOKEN` (hoy en dos bloques EN EL TALLER / POR LLEGAR, salta a mañana, refresh 60 s sin JS) + KM `required` y ACABADO → `POST /taller/acabar` + NO SE HACE con motivo → `POST /taller/incidencia` — ver sección propia |
| Ciclo de estados | ✅  | confirmada → atendida → acabada → llamado → pagada (+ cancelada e incidencia con `motivo`), botón ✓ en el listado, banda "N coches acabados / con incidencia" con sondeo 10 s, fecha `DD/MM/YYYY`, **cuatro vistas de listado + calendario semanal** (`?ver=`) y **panel instalable con badge** en el icono — ver "Modelo de cita", "Panel /admin" y "Vista de calendario" |
...
- **Comentarios obsoletos en `server.js`** (solo comentarios, el código es correcto; corregir en la próxima sesión que toque esa zona). Verificado el 04/09/2026: los dos de "30 s" **ya no existen**. Quedan **DOS**:
  - **Línea 2277** (sobre `GET /admin/recordatorios`): dice que `isSameOrigin` "solo aplica a POST/DELETE"; ya incluye PUT.
  - **Líneas 1588-1594** (comentario de `tallerHTML`, antes de `const tarjeta`): describe un botón ACABADO que en `confirmada` "se MUESTRA pero DESACTIVADO ('SIN LLEGAR')". **Ya no es así**: el código pinta en POR LLEGAR solo el texto "Aún no ha llegado", sin botón ni formulario. Reescribir el párrafo; el resto del comentario (KM, NO SE HACE, POR LLEGAR) sí está al día.
...
- La pantalla `/taller` es **pública por diseño**: cualquier dato nuevo que se añada ahí debe pasar el filtro "¿puede verlo un cliente que espera?". Precio, apellidos y teléfono, **nunca**. Los km sí se muestran (campo KM de las `atendida`): asumido como dato no sensible.
- **Eficiencia en las sesiones de Claude Code**: los prompts incluyen la **referencia exacta** (id, función o símbolo) de lo que hay que tocar, para que busque con grep en vez de explorar o leer archivos enteros (`server.js` pasa de 2.400 líneas). Para **cambios triviales se aplica directamente, sin diff previo**. Y se pide **respuesta corta**: por defecto redacta reportes largos que gastan contexto sin aportar.
