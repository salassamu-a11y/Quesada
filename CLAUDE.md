...
- **`hechoPor`** (09/2026): **opcional**, valor **CERRADO** `'D'` o `'J'` — quién hizo el trabajo. Constante **`MECANICOS = { D: 'Dani', J: 'Jorge' }`** y helper **`nombreMecanico(letra)`**, que devuelve `''` si la letra no es válida (usa `hasOwnProperty`: un `'constructor'` o `'toString'` en el JSON no debe casar). **Solo lo escribe `POST /taller/acabar`** (los dos botones D/J de la pantalla del taller): no está en `camposVehiculo` ni en el formulario del panel, así que el `PUT /admin/cita/:id` lo **conserva** sin tocarlo. El panel lo **muestra** ("Hecho por Dani" bajo el servicio) y lo copia al Excel. **No se pide en la incidencia**: si el trabajo no se hace, no hay quién lo haya hecho. Las citas anteriores no lo tienen; `undefined` tolerado.
...
| POST   | /taller/acabar                    | **PÚBLICA, fuera del bloque `/admin`**: escritura desde la pantalla del taller. Body form-urlencoded `k` (mismo `TALLER_TOKEN`) + `id` + `kilometros` y/o `precio` + `hechoPor` (los tres opcionales para el servidor; el precio lo manda la pantalla en los servicios de `servicioConPrecio()`). Solo pasa a `acabada`, solo desde `confirmada`/`atendida` y **solo si la cita es de HOY**; guarda km, precio y `hechoPor` si vienen (inválidos → 409; `hechoPor` debe ser una letra de `MECANICOS`). Token malo → 404 genérico en texto plano; cita inexistente → 404 HTML; cualquier otro caso → 409 HTML; éxito → 302 a `/taller?k=` (ver "Pantalla del taller") |
...
| GET    | /admin                            | Panel HTML con tabla de citas (auth básica). `?ver=` elige una de **cuatro vistas de listado** (`proximas` por defecto, `hoy`, `llamar`, `todas`) o **`calendario`** (rejilla semanal, admite `&semana=YYYY-MM-DD`); valor desconocido → `proximas` vía `resolverVista()` (ver "Panel /admin — vista de citas" y "Vista de calendario") |
...
- **Vistas del listado (`?ver=`)**: objeto **`VISTAS`** (server.js ~línea 806, clave → etiqueta; el orden del objeto es el orden de las opciones) con **cuatro** vistas de listado. **Solo filtro de visualización**: `citas.json` no se toca. La antigua vista `mes` fue **SUSTITUIDA** por `calendario` (ver "Vista de calendario").
...
- **Desplegable de estado en el formulario** (09/2026): `#nc-estado` (envuelto en `#nc-estado-wrap`), **a la derecha de Forma de pago** y visible **SOLO en modo edición** — en el alta no aparece, porque las citas nacen en `confirmada` y no tiene sentido elegirlo. Mismas opciones que el desplegable de la fila; `pendiente` se inserta al vuelo solo si la cita ya está en él (`editarCita`).
  - **Cambia una decisión anterior: el `PUT` ya NO conserva siempre el estado.** Si el body trae uno de `ESTADOS_VALIDOS` lo aplica; si no viene o viene vacío, lo conserva; si viene y no es válido → 400.
  - **`ESTADOS_VALIDOS` y `aplicarEstado(cita, estado)` se extrajeron para esto**: la escritura/borrado de `pagadaEn` vive en **un solo sitio**, usado por el `PUT` y por `POST /admin/cita/:id/estado`. Así una cita marcada `pagada` desde el formulario no se queda sin hora de cobro.
...
- **9 opciones en el desplegable** del formulario de nueva cita: Pinchazo turismo, Pinchazo furgoneta, Pinchazo moto, **Rueda suelta**, Montaje de neumáticos, Alineado, Cruce, Equilibrado, Válvulas TPMS. **Sustituyen a las 5 anteriores.**
- **"Rueda suelta"** (09/2026): el cliente **trae solo la rueda, sin el coche**. Va tras los tres pinchazos en el desplegable y, como ellos, entra en **`SERVICIOS_CON_PRECIO`**: en la pantalla del taller el campo que acompaña a ACABADO es el **PRECIO**, no los km (ver "Pantalla del taller").
...
Vista para una pantalla colgada en el taller, encendida todo el día y visible por cualquiera que pase. **Solo lectura salvo DOS acciones acotadas** en las tarjetas del bloque EN EL TALLER: "ACABADO" (`POST /taller/acabar`) y "NO SE HACE" (`POST /taller/incidencia`), ver abajo. `tallerHTML(citas, fecha, esManana, token)`.
...
- **Detalle recortado a DOS LÍNEAS** (`.detalle` con `-webkit-line-clamp: 2` + `overflow:hidden`, puntos suspensivos); **el texto completo va en el `title` de la tarjeta**. **Motivo**: `detalle` admite **300 caracteres** y a ese tamaño de fuente serían 4-5 líneas, la tarjeta crecería y **las citas de abajo saldrían del viewport** de la pantalla fija.
...
- **Acciones de la tarjeta (`.acciones`) — SOLO en el bloque EN EL TALLER** (`atendida`) de la vista de HOY (`conBoton = !esManana && !!token`). **SIN JS a propósito**: la pantalla pasa semanas abierta y no puede depender de que un script siga vivo. **Sin confirmación** ("¿estás seguro?"): en un taller es fricción, y el error se corrige en dos clics desde el panel. Dos filas:
  - **Fila 1 — campo "KM" + botones "D"/"J" + botón "ACABADO"**: `<form method="post" action="/taller/acabar">` con `k` (token) e `id` en `hidden` y el input `kilometros` (`type="text"` + `inputmode="numeric"` + `pattern="[0-9]{1,7}"`, **NO `type="number"`**: se come los ceros a la izquierda y en móvil da problemas) con **`required`**: los mecánicos apuntan los km al terminar y se les olvida, así que el navegador exige el campo antes de enviar. Sale prerrellenado si la cita ya trae km. **CRÍTICO: el `required` es SOLO del HTML — el servidor NO exige kilómetros, A PROPÓSITO**: si la tablet falla, el cuentakilómetros no arranca o es una moto sin marcador, nadie puede quedarse atrapado sin poder marcar el coche; Vicky lo arregla desde el panel. **NO añadir validación de obligatoriedad en `POST /taller/acabar`.**
    ...
    - **Selector "D" / "J" (`hechoPor`) junto al campo**: dos `<input type="radio">` con el mismo `name` **ocultos** y sus `<label>` con aspecto de botón, **SIN JS** (`:checked` pinta el elegido en amarillo; mismo patrón que el checkbox del motivo). Botones y no campo de texto: se pulsa con las manos sucias y nadie escribe "d" o "Dani". **Ninguno preseleccionado a propósito**, y `required` con el mismo criterio que los km (**solo del HTML**; ver `hechoPor` en "Modelo de cita").
  ...
  - **Maquetación**: `.acciones` es un **grid de dos columnas fijas** (`14rem 13rem`: campo | botón) y los `.accion` (los dos `<form>`) usan **`display: contents`**, para que elementos de dos formularios distintos se alineen en las mismas columnas (KM y motivo comparten columna; ACABADO, NO SE HACE y CONFIRMAR quedan alineados a la derecha). Anchos fijos a propósito: al desplegar el motivo nada se mueve. `position: relative` contiene al checkbox oculto.
...
| Pantalla taller | ✅   | `GET /taller?k=TOKEN` (hoy en dos bloques EN EL TALLER / POR LLEGAR, salta a mañana, refresh 60 s sin JS) + KM y botones **D/J** (`hechoPor`) `required` y ACABADO → `POST /taller/acabar` (en los servicios de **`servicioConPrecio()`** —pinchazos y **Rueda suelta**— el campo es el **PRECIO**, siempre vacío) + NO SE HACE con motivo → `POST /taller/incidencia` (sin D/J) + detalle recortado a dos líneas + **instalable** como app (`GET /taller/manifest.json` con token) — ver sección propia |
| Ciclo de estados | ✅  | confirmada → atendida → acabada → llamado → pagada (+ cancelada e incidencia con `motivo`), botón ✓ en el listado, banda "N coches acabados / con incidencia" con sondeo 10 s, fecha `DD/MM/YYYY`, **cuatro vistas de listado + calendario semanal** (`?ver=`) y **panel instalable con badge** en el icono. Desde el 04/09/2026: campo `pago` (hoja de papel replicada; `tipoVehiculo` y `factura` se añadieron y **se retiraron** después a petición del cliente), cerradas al final del listado, vuelta a la misma vista y posición al cambiar estado, alta y edición desde el calendario, buscador por nombre/teléfono/matrícula y panel usable en móvil. Después: **acciones en iconos** (tabla sin scroll a 1366 px) y **Copiar al Excel** (por fila y "Copiar todas", **13 columnas** del `Diario.ods` por tabulación). En septiembre de 2026: **desplegable de pago en la fila** (`POST /admin/cita/:id/pago` por fetch, `w-28`), **estado editable desde el formulario** (`ESTADOS_VALIDOS` + `aplicarEstado`), **`pagadaEn`** ordenando las pagadas por orden de cobro, **`hechoPor` (D/J)**, **dos teléfonos por cita**, `detalle` a 300 caracteres y servicio **"Rueda suelta"** — ver "Modelo de cita", "Panel /admin" y "Vista de calendario" |
...
- La pantalla `/taller` es **pública por diseño**: cualquier dato nuevo que se añada ahí debe pasar el filtro "¿puede verlo un cliente que espera?". Precio, **forma de pago**, apellidos y teléfono, **nunca**. **Matiz del precio**: en los servicios de `servicioConPrecio()` (pinchazos y "Rueda suelta") aparece un campo "€" para que el mecánico lo escriba, pero **siempre VACÍO**, jamás prerrellenado con el importe guardado; mostrar un precio existente sigue prohibido. Los km sí se muestran (campo KM de las `atendida`, prerrellenado): asumido como dato no sensible.
